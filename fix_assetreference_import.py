"""
Fix for: ImportError: cannot import name 'AssetReference' from 'app.assets.database.models'

Usage:
    python fix_assetreference_import.py [path_to_ComfyUI]

If no path is given, the script looks for ComfyUI in the current directory.
"""
...
def find_models_py(comfyui_root: str) -> str | None:
    target = os.path.join(comfyui_root, "app", "assets", "database", "models.py")
    return target if os.path.isfile(target) else None


def main() -> None:
    comfyui_root = sys.argv[1] if len(sys.argv) > 1 else os.getcwd()
    comfyui_root = os.path.abspath(comfyui_root)

    models_path = find_models_py(comfyui_root)
    if models_path is None:
        print(f"ERROR: Could not find app/assets/database/models.py under {comfyui_root}")
        print("Please pass the ComfyUI root directory as an argument:")
        print("  python fix_assetreference_import.py D:\\ComfyUI-aki\\ComfyUI-aki-v3\\ComfyUI")
        sys.exit(1)

    # Check if already fixed
    with open(models_path, encoding="utf-8") as f:
        existing = f.read()
    if "class AssetReference" in existing:
        print(f"AssetReference already present in {models_path} — nothing to do.")
        sys.exit(0)
    ...
    print("You can now restart ComfyUI.")
